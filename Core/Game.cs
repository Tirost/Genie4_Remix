                    ...
                    case "roundTime":
                        {
                            ...
                            if (int.TryParse(GetAttributeData(oXmlNode, argstrAttributeName24), out int parsedRt))
                            {
                                m_iRoundTime = parsedRt;
                                m_oGlobals.XmlRoundTimeTarget = parsedRt;
                            }
                            break;
                        }
                    ...
                    case "prompt":
                        {
                            ...
                            if (int.TryParse(GetAttributeData(oXmlNode, argstrAttributeName25), out m_iGameTime))
                            {
                                ...
                                m_oGlobals.ServerGameTime = m_iGameTime;
                                m_oGlobals.ServerTimeSyncUtc = DateTimeOffset.UtcNow.ToUnixTimeSeconds();

                                int rt = 0;
                                if (m_oGlobals.Config.bXmlRoundTime)
                                {
                                    if (m_iRoundTime > m_iGameTime)
                                    {
                                        rt = m_iRoundTime - m_iGameTime;
                                        SetRoundTime(rt);
                                        if (m_bStatusPromptEnabled == false && (m_oGlobals.Config.PromptForce == true))
                                            strBuffer += "R";
                                        int rtWithOffset = rt + Convert.ToInt32(m_oGlobals.Config.dRTOffset);
                                        if (rtWithOffset < 0) rtWithOffset = 0;
                                        m_oGlobals.VariableList.Add("roundtime", rtWithOffset.ToString(), Globals.Variables.VariableType.Reserved);
                                    }
                                    else
                                    {
                                        m_iRoundTime = 0;
                                        m_oGlobals.XmlRoundTimeTarget = 0;
                                        m_oGlobals.VariableList.Add("roundtime", "0", Globals.Variables.VariableType.Reserved);
                                    }
                                }
                                else
                                {
