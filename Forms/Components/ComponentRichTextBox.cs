...
namespace GenieClient
{
    public partial class ComponentRichTextBox : RichTextBox
    {
        ...
        private void InvokeAddRTF(string text)
        {
            ...
            else if (TextLength > MaxBufferSize) // Flush buffer when user isn't scrolling.
            {
                bIsFlushingBuffer = true;
                BeginUpdate();
            ...
            bool bNeedUpdateLock = m_bIsScrolling && !bIsFlushingBuffer;
            if (bNeedUpdateLock)
            {
                BeginUpdate();
            }
            ...
            {
                if (bIsFlushingBuffer || bNeedUpdateLock)
                {
                    EndUpdate();
                }
            }
        ...
        private void SetLinks(int offset)
        {
            if (LinkList.Count > 0)
            {
                bool bNeedUpdateLock = m_bIsScrolling;
                if (bNeedUpdateLock)
                {
                    BeginUpdate();
                }
